...
if __name__ == '__main__':
    ...
    docker_manager = DockerSimManager('docker.pkg.github.com/michaelwittmann/docker_simulation_pipeline_example/random-art-image',
                                      ...
                                      sim_path)
    for i in range(1,10):
        docker_manager.add_sim_job(SimJob(f'size{i}x{i}',None, command=f'-g {i} -i 30 -s 5000 -o arts -n 50'))

    for i in range(1,10):
        docker_manager.add_sim_job(SimJob(f'invaders_{i}',None, command=f'-g 10 -i {i} -s 5000 -o arts -n 50'))
